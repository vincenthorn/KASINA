    ...
    try {
      ...
      responseChar.addEventListener('characteristicvaluechanged', (event: any) => {
        ...
        try {
          ...
          // The belt is connected but not sending force readings yet
          // We'll leave the visualization static until we get actual force readings
          
          const now = Date.now();
          
          // Keep the visualization still by not updating the breath data
          // This ensures the orb remains static when there's no real data
          
          // We're deliberately NOT generating any simulated breath pattern
          // The orb will stay at its default size until actual data is received
          
          // Only set a timestamp so the app knows we're connected
          localStorage.setItem('latestBreathTimestamp', now.toString());
          
          // Keep track of the real connection status
          localStorage.setItem('realDataAvailable', 'true');
          localStorage.setItem('breathDataSource', 'real');
          
          // Use a fixed, normal breathing rate for display only
          // This won't affect the visualization since we're not simulating data
          localStorage.setItem('breathingRate', '10');
          ...
          if (now % 2000 < 100) { // Log every 2 seconds to avoid flooding
            ...
            // Get the current breath reading for logging
            const currentForce = parseFloat(localStorage.getItem('latestBreathReading') || '0');
            const currentBPM = parseInt(localStorage.getItem('breathingRate') || '10');
            
            // Log the current breath status
            addLog(`Current breath data: Force=${currentForce.toFixed(2)}N, BPM=${currentBPM}`);
          }
