...
Changelog:
- February 26, 2026: Added "Absorbed in Red" guided meditation for Red Kasina (17 min). Updated email template: replaced yellow PNG logo with inline CSS solid white circle orb, added prominent "KASINA" heading, reduced spacing. Fixed hasGuidedMeditation/getGuidedMeditation to handle undefined keys safely. Removed "KASINA is freely offered by the Pragmatic Dharma Sangha" text from registration page. Switched primary hosting from Render.com to Replit deployment. Added comprehensive diagnostic logging across Vernier breath rate data pipeline (useVernierBreathManual, BreathKasinaOrb, BreathRateChart) for troubleshooting live BPM display and post-session chart.
