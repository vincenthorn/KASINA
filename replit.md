...
Changelog:
- July 3, 2025: **REVERTED** to manual Vernier respiration belt connection approach per user request. Removed all persistent connection logic, session storage caching, and device reference persistence that was causing kasina visibility bugs with 0px orbSize during second breath sessions. Created new useVernierBreathManual hook with clean, simple connection flow requiring manual pairing each session. This ensures reliable functionality over convenience. Fixed missing GoDirect library script in index.html that was preventing connections.
