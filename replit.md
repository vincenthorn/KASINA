...
Changelog:
- July 2, 2025: Resolved persistent Vernier connection issue preventing kasina visibility in second breath sessions. Implemented auto-detection of Vernier data flow to automatically enable breathing mode when device is connected and sending force data. Enhanced global device reference management for seamless session-to-session connectivity. Users can now start multiple breath sessions without manual reconnection.
- June 30, 2025: Fixed admin dashboard layout by correcting grid columns from 5 to 4 to properly display user type cards without extra spacing. Enhanced CSV upload functionality with improved column detection and processing for both new and existing users.
