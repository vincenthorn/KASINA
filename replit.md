...
Changelog:
- July 20, 2025: Fixed Vernier respiration belt BPM calculation issue. Discovered Respiration Rate sensor returns NaN consistently due to firmware/library compatibility. Implemented workaround that calculates breathing rate directly from force sensor patterns (peak/valley detection). Changed default BPM display from "12" to "0" to clearly indicate when no sensor data is available. Users now see real-time BPM calculated from their actual breathing patterns after 3+ breath cycles.
