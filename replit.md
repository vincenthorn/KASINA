...
Changelog:
- February 26, 2026: Fixed Vernier breath rate live display not working across page navigations. Root cause: useVernierBreathManual hook created independent instances per page, losing isConnected state when navigating. Fix: hook now auto-adopts existing window.vernierDevice on mount, re-attaches sensor listeners, and resumes session timer. Added proper listener cleanup on unmount (prevents stale handlers). Reduced excessive per-frame console.log noise from kasina scaling. Also added "Absorbed in Red" guided meditation, updated email template, fixed guidedMeditations key safety, removed Sangha text from registration, switched hosting to Replit, added diagnostic logging.
