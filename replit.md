...
Changelog:
- February 26, 2026: Fixed three BPM display bugs and added breath rate history persistence. (1) Force-computed BPM now always updates every 10s — removed guard that stopped updates after first sensor reading. (2) effectiveUseVernier now latches true permanently once detected, preventing BPM display flickering. (3) BPM display guards against showing 0 or default 12. (4) Added breath_rate_data column to sessions table — breath rate history saved to DB on session end and retrievable via GET /api/sessions/:id. (5) Created SessionDetailPage at /reflection/session/:id with Recharts LineChart showing BPM over time plus summary stats. (6) Breath Kasina entries in PracticeLog are now clickable, navigating to session detail page. Also fixed connectionStartTimeRef initialization so history records even when useVernier state is false but force data flows.
